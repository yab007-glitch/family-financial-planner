...
export class WealthOptimizer {
  ...
  private marginalRate: number;

  constructor(income: number, age: number, province = 'QC', numChildren = 0, childrenAges: number[] = []) {
    if (income < 0) throw new Error('Income cannot be negative');
    if (age < 0) throw new Error('Age cannot be negative');
    if (numChildren < 0) throw new Error('Number of children cannot be negative');
    this.income = income;
    this.age = age;
  ...
  getOptimalStrategy(availableCash: number) {
    if (availableCash < 0) throw new Error('Available cash cannot be negative');
    ...
    if (this.marginalRate < 0.30) {
      recommendations.push({
        priority: 2,
        account: 'TFSA',
        action: `Max TFSA: $${Math.min(availableCash, tfsaRoom).toLocaleString()}`,
        reason: `Your marginal tax rate is ${(this.marginalRate * 100).toFixed(1)}% (< 30%). TFSA withdrawals are tax-free, no forced withdrawals at 71, won't affect OAS/GIS clawbacks.`,
        ...
        annualLimit: 7000,
        lifetimeLimit: tfsaRoom,
      });
    } else {
      const rrspLimit = (taxInfo as any).rrspContribution || RRSP_MAX_CONTRIBUTION_2025;
      recommendations.push({
        priority: 2,
        account: 'RRSP',
        action: `Max RRSP: $${Math.min(availableCash, rrspLimit).toLocaleString()}`,
        reason: `Your marginal rate is ${(this.marginalRate * 100).toFixed(1)}%. Every $1,000 RRSP = $${(this.marginalRate * 1000).toFixed(0)} tax refund. Reinvest refund for double benefit.`,
        amount: Math.min(availableCash, rrspLimit),
        benefit: `Tax refund of $${(Math.min(availableCash, rrspLimit) * this.marginalRate).toFixed(0)}`,
        returnOnInvestment: `${(this.marginalRate * 100).toFixed(1)}% immediate return via tax refund + tax-deferred growth`,
        urgency: 'HIGH - Tax shield on high income',
        annualLimit: rrspLimit,
        reinvestRefund: true,
      });
    }
    ...
    if (this.age >= 18 && this.age <= 71) {
      recommendations.push({
        ...
        benefit: `Tax deduction worth $${(Math.min(8000, availableCash) * this.marginalRate).toFixed(0)} + tax-free growth`,
        returnOnInvestment: `${(this.marginalRate * 100).toFixed(1)}% tax deduction + tax-free withdrawal for home purchase`,
    ...
    if (this.marginalRate < 0.30 && availableCash > tfsaRoom) {
      recommendations.push({
        priority: 4,
        account: 'RRSP (deferred)',
        action: `Contribute to RRSP up to limit: $${Math.min(availableCash - tfsaRoom, RRSP_MAX_CONTRIBUTION_2025).toLocaleString()}`,
        ...
        amount: Math.min(availableCash - tfsaRoom, RRSP_MAX_CONTRIBUTION_2025),
    ...
    return {
      income: this.income,
      marginalRate: this.marginalRate,
      taxInfo,
      recommendations,
    };
  }
  ...
  calculateQuebecCredits(familyIncome: number, numChildren: number, isSingleParent: boolean, _rentPaid = 0, _propertyTax = 0) {
    if (familyIncome < 0) throw new Error('Family income cannot be negative');
    if (numChildren < 0) throw new Error('Number of children cannot be negative');
    const credits: any[] = [];
    ...
    if (numChildren > 0) {
      ...
      credits.push({
        ...
        monthlyAmount: (ccb as any).monthlyBenefit,
        annualAmount: (ccb as any).annualBenefit,
        eligible: (ccb as any).annualBenefit > 0,
  ...
  getWealthRoadmap(availableCash: number, goals: any = {}) {
    if (availableCash < 0) throw new Error('Available cash cannot be negative');
    const strategy = this.getOptimalStrategy(availableCash);
    const credits = this.calculateQuebecCredits(this.income, this.numChildren, goals.isSingleParent ?? false, goals.rentPaid, goals.propertyTax);
    ...
    return {
      ...
      totalEstimatedBenefits: credits.reduce((sum: number, c: any) => {
  ...
  private buildYearOnePlan(recommendations: WealthRecommendation[], availableCash: number) {
      ...
      if (rec.amount) {
        ...
        if (allocation > 0) {
          plan.push({
            ...
            amount: allocation,
            benefit: rec.benefit,
            remainingCash: Math.round((remaining - allocation) * 100) / 100,
          });
    ...
    if (remaining > 0) {
      plan.push({
        ...
        account: 'Overflow',
        amount: remaining,
        remainingCash: Math.round(remaining * 100) / 100,
